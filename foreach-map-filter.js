...
function showFirstAndLast(arr){
    const firstLast = [];
    arr.forEach(function(item){
        firstLast.push(item[0] + item[item.length - 1]);
        })
    return firstLast;
}
...
function doubleValuesWithMap(arr) {
    const doubledVal = arr.map(function(val){return val * 2});
    return doubledVal;
}
...
function valTimesIndex(arr){
    const indexedArr = arr.map(function(value, index){
        return value * index;
    })
    return indexedArr;
}
...
function extractKey(arr, key){
    const valArr = arr.map((obj) => obj[key] );
    return valArr;
}
...
function filterByValue(arr, key) {
    const objWithKey = arr.filter(function(obj){
...
function removeVowels(str) {
    const LowerCasedArr = Array.from(str.toLowerCase());
    const removedVow =  (LowerCasedArr.filter(function(value){
        return 'aeiou'.indexOf(value) === -1;
    }));
    return removedVow.join('');
}
...
function doubleOddNumbers(arr) {
    const oddNum = arr.filter(function(value){
        return value % 2 !== 0;
    })
    return oddNum.map(function(value){
        return value * 2;
    })
}
